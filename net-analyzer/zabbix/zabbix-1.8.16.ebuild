...
src_install() {
	...
	if use server; then
		insinto /etc/zabbix
		doins \
			"${FILESDIR}/1.8.16"/zabbix_server.conf \
			"${FILESDIR}/1.8.16"/zabbix_trapper.conf
		doinitd \
			"${FILESDIR}/1.8.16"/init.d/zabbix-server
		dosbin \
	...
	if use proxy; then
		doinitd \
			"${FILESDIR}/1.8.16"/init.d/zabbix-proxy
		dosbin \
			src/zabbix_proxy/zabbix_proxy
		insinto /etc/zabbix
		doins \
			"${FILESDIR}/1.8.16"/zabbix_proxy.conf
		dodir \
			/usr/share/zabbix/database
		insinto /usr/share/zabbix/database
		doins -r \
	...
	if use agent; then
		insinto /etc/zabbix
		doins \
			"${FILESDIR}/1.8.16"/zabbix_agent.conf \
			"${FILESDIR}/1.8.16"/zabbix_agentd.conf
		doinitd \
			"${FILESDIR}/1.8.16"/init.d/zabbix-agentd
		dosbin \
	...
	if use frontend; then
		...
		webapp_postinst_txt en "${FILESDIR}/"1.8.16/postinstall-en.txt
		webapp_configfile \
